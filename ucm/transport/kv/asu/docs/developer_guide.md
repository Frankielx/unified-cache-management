...
│   │  - Endpoint Sharing  │ ref│  - DeleteConnections          │  │
│   │                      │    │  - Send                       │  │
│   │  ┌────────────────┐  │    │  - RegisterMemory             │  │
...
ConnectionManager holds a `TransProvider&` reference and directly calls its methods (CreateConnection, DeleteConnections) instead of using function pointer callbacks. This simplifies the API and makes the dependency explicit.
...
class ConnectionManager {
    ...
    // TransProvider reference for connection lifecycle
...
for each channel in to_recover:
    if inflight > 0: put back to drainList, continue waiting
        ...
        provider_.CreateConnection(localIp_, ep.ip, ep.port, 1, timeout_, handles)
...
| `channelCacheMu_` | channelCache_ | Worker/Select | std::mutex |
...
|-----------|---------|-------------|
| providerType | AICPU | TransProvider implementation type (enum: AICPU) |
...
### 4.1 TransProvider Selection

`AsuTransportImpl` creates the appropriate `TransProvider` implementation based on `config.providerType` during `Init()`:
...
For unit testing, `SetTransProvider()` (private method) can inject a stub provider before `Init()`:

```cpp
// Test code:
transport.SetTransProvider(std::make_unique<StubTransProvider>());
transport.Init(config);  // Uses injected stub instead of creating AICPUTransProvider
```
...
Header files:
- `trans/include/hixl_kernel/` — HixlSend kernel interface header (`hixl_send.h`), included in the project
- `acl/` — CANN ACL headers, found from CANN installation via `ASU_ASCEND_INCLUDE_DIR`
- `hcomm/` — HCOMM communication library headers, found from CANN installation via `ASU_ASCEND_ARCH_INCLUDE_DIR`

Dynamic library dependencies (requires CANN package installation):
- `libhcomm.so` — HCOMM communication library, found via `ASU_HCOMM_LIB`
- `libascendcl.so` — CANN ACL runtime library, found via `ASU_ASCENDCL_LIB`
...
**Method 2: Explicitly Specify CANN Path**
...
cmake \
  ...
  -DBUILD_UCM_ASU=ON \
  -DBUILD_PROVIDER_DEMO=ON \
  -DASCEND_ROOT=/usr/local/Ascend/cann-9.1.0-beta.1 \
  /path/to/unified-cache-management/ucm/transport/kv
...
- CMakeLists.txt uses `find_path`/`find_library` to locate ACL and hcomm headers/libraries from the CANN installation path specified by `ASCEND_ROOT`
- If `ASCEND_ROOT` is not specified, it defaults to `/usr/local/Ascend/ascend-toolkit/latest`
...
- CANN headers and libraries are located via `ASCEND_ROOT` (defaults to `/usr/local/Ascend/ascend-toolkit/latest`), or can be overridden by passing `-DASCEND_ROOT=/path/to/cann`
