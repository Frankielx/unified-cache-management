...
AICPUTransProvider::AICPUTransProvider(const std::string& kernelJsonPath,
    ...
    if (aclRet != ACL_SUCCESS) {
        UC_ERROR("AICPUTransProvider::AICPUTransProvider: aclrtBinaryLoadFromFile failed, ret={}", aclRet);
        return;
    }

    aclRet = aclrtBinaryGetFunction(kernelBin_, "HixlSend", &kernelFunc_);
    if (aclRet != ACL_SUCCESS) {
        UC_ERROR("AICPUTransProvider::AICPUTransProvider: aclrtBinaryGetFunction failed, ret={}", aclRet);
        kernelFunc_ = nullptr;
    }
}
...
void AICPUTransProvider::LoadIpToDeviceMap() {
    ...
    if (!fp) {
        UC_WARN("AICPUTransProvider::LoadIpToDeviceMap: failed to open {}, using default mapping", mapFile);
        ipToDeviceMapLoaded_ = true;
        return;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        char ip[64] = {};
        uint32_t devId = 0;
        if (sscanf(line, "%63s %u", ip, &devId) == 2) {
            ipToDeviceMap_[ip] = devId;
        }
    }
    fclose(fp);

    ipToDeviceMapLoaded_ = true;
    UC_INFO("AICPUTransProvider::LoadIpToDeviceMap: loaded {} entries from {}", ipToDeviceMap_.size(), mapFile);
}

uint32_t AICPUTransProvider::LookupDeviceByIp(const std::string& ip) {
    LoadIpToDeviceMap();
    auto it = ipToDeviceMap_.find(ip);
    if (it != ipToDeviceMap_.end()) {
        return it->second;
    }
...
void* AICPUTransProvider::GetOrCreateEndpoint(const std::string& localIp) {
    ...
    if (endpoint_ != nullptr) {
        if (localIp_ != localIp) {
            UC_ERROR("AICPUTransProvider::GetOrCreateEndpoint: localIp={} conflicts with existing localIp={}",
                     localIp, localIp_);
            return nullptr;
        }
        endpointRefCount_++;
    ...
    if (ret != 0) {
        UC_ERROR("AICPUTransProvider::GetOrCreateEndpoint: HcommEndpointCreate failed, ret={}", ret);
        return nullptr;
    }
...
void AICPUTransProvider::ReleaseEndpoint(const std::string& localIp) {
    ...
    if (endpoint_ == nullptr || localIp_ != localIp) {
        return;
    }
    ...
    std::vector<ConnectionHandle>& connectionHandles)
{
    ...
    for (uint32_t i = 0; i < qpNum; ++i) {
        ...
        if (aclRet != ACL_SUCCESS) {
            UC_ERROR("AICPUTransProvider::CreateConnection: aclrtCreateStream failed, ret={}", aclRet);
    ...
    uint32_t quietCount)
{
    ...
    for (const auto& batch : ioBatches) {
        ...
        if (!kernelFunc_ || !ctx->stream) {
            results.push_back(Status::Error(StatusCode::NOT_INITIALIZED, "Kernel or stream not ready"));
            continue;
        }
        ...
        if (aclRet != ACL_SUCCESS) {
            ...
            results.push_back(Status::Error(StatusCode::INTERNAL_ERROR, "KernelArgsFinalize failed"));
            continue;
        }
        ...
        aclRet = aclrtLaunchKernelWithConfig(kernelFunc_, 1, ctx->stream, &cfg, argsHandle, nullptr);
        if (aclRet != ACL_SUCCESS) {
        ...
        if (aclRet != ACL_SUCCESS) {
            ...
            results.push_back(Status::Error(StatusCode::INTERNAL_ERROR, "SynchronizeStream failed"));
            continue;
        }
    ...
    const std::vector<UnregisterMemoryDesc>& memoryDescs)
{
    ...
    for (const auto& desc : memoryDescs) {
        if (desc.connectionHandle != nullptr) {
            ...
            if (!ctx) {
                results.push_back(Status::Error(StatusCode::INVALID_ARGUMENT, "Invalid connection handle"));
                continue;
            }
        }
...
Status AICPUTransProvider::GetMemTokenId(
    MemHandle memHandle,
    uint32_t& tokenId)
{
    if (!memHandle) {
...
Status AICPUTransProvider::ExportMemory(
    ConnectionHandle connectionHandle,
    MemHandle memHandle,
    void** exportDesc,
    uint32_t* exportLen)
...
Status AICPUTransProvider::ImportMemory(
    ConnectionHandle connectionHandle,
    const void* importDesc,
    uint32_t importLen,
...
Status AICPUTransProvider::GetImportedMemoryInfo(
    ConnectionHandle connectionHandle,
    MemHandle importedHandle,
    uint64_t* addr,
    uint64_t* size)
...
Status AICPUTransProvider::UnimportMemory(
    ConnectionHandle connectionHandle,
    MemHandle importedHandle)
{
    if (connectionHandle != nullptr) {
    ...
    HcommResult ret = HcommMemUnimport(endpoint_, memDesc.data(), static_cast<uint32_t>(memDesc.size()));
    if (ret != 0) {
