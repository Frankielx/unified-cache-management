                                             ...
                                             uint32_t kernelCount, uint32_t quietCount)
{
    ...
    for (const auto& batch : ioBatches) {
        ...
        void* contextBuf = nullptr;
        if (batch.remoteMailboxAddr != 0) {
            ...
            sendRecvContext.transportChannel = ctx->channel;
            sendRecvContext.localMailboxAddr = reinterpret_cast<uint64_t>(batch.sendBuffer);
            sendRecvContext.localMailboxSize = batch.len + sizeof(HcommSendRecvMailboxHeader);
            sendRecvContext.remoteMailboxAddr = batch.remoteMailboxAddr;
            sendRecvContext.remoteMailboxSize = batch.remoteMailboxSize;

            aclError aclAlloc = aclrtMalloc(&contextBuf, sizeof(sendRecvContext),
                                            ACL_MEM_MALLOC_HUGE_ONLY);
            if (aclAlloc != ACL_SUCCESS) {
                UC_ERROR("AICPUTransProvider::Send: malloc context failed, ret={}", aclAlloc);
                results.push_back(
                    Status::Error(StatusCode::INTERNAL_ERROR, "malloc context failed"));
                continue;
            }

            aclError aclCopy = aclrtMemcpy(contextBuf, sizeof(sendRecvContext), &sendRecvContext,
                                           sizeof(sendRecvContext), ACL_MEMCPY_HOST_TO_DEVICE);
            if (aclCopy != ACL_SUCCESS) {
                UC_ERROR("AICPUTransProvider::Send: memcpy context failed, ret={}", aclCopy);
                aclrtFree(contextBuf);
                results.push_back(
                    Status::Error(StatusCode::INTERNAL_ERROR, "memcpy context failed"));
                continue;
            }

            args.channel = reinterpret_cast<uint64_t>(contextBuf);
            args.local_src = static_cast<uint8_t*>(batch.sendBuffer) +
                             sizeof(HcommSendRecvMailboxHeader);
            args.len = batch.len;
        }
        ...
        if (aclRet != ACL_SUCCESS) {
            ...
            if (contextBuf) { aclrtFree(contextBuf); }
