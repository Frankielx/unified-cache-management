...
public:
    ...
    virtual std::vector<Status> Send(
    ...
    // 线程管理
    virtual Status AllocThread(
        uint32_t threadNum,
        const std::vector<uint32_t>& notifyNumPerThread,
        std::vector<ThreadHandle>& threads) = 0;

    virtual std::vector<Status> FreeThread(
        const std::vector<ThreadHandle>& threads) = 0;

    // 获取内存的 token ID
    virtual Status GetMemTokenId(
        MemHandle memHandle,
        uint32_t& tokenId) = 0;
};
