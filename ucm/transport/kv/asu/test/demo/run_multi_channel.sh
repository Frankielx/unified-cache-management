...
BUILD_DIR="/home/lx/test/master/asu-aicpu/unified-cache-management/build/asu"
IP_MAP="/tmp/npu_ip_map.txt"

BINARY="${BUILD_DIR}/aicpu_multi_channel_demo"
LOG_DIR="${SCRIPT_DIR}/logs_multi_channel"

cleanup() {
    rm -f /tmp/r0_multi.bin /tmp/r1_multi.bin /tmp/hixl_multi.done /tmp/npu_ip_map.txt
}
trap cleanup EXIT
cleanup
...
(
    ...
    "${BINARY}" \
        ...
        --local-file=/tmp/r1_multi.bin --peer-file=/tmp/r0_multi.bin \
        --done-file=/tmp/hixl_multi.done \
...
(
    ...
    "${BINARY}" \
        ...
        --local-file=/tmp/r0_multi.bin --peer-file=/tmp/r1_multi.bin \
        --done-file=/tmp/hixl_multi.done \
        --kernel-json="${KERNEL_JSON}" \
        --ip-map="${IP_MAP}"
)
...
cat /root/ascend/log/debug/device-0/* > "${LOG_DIR}/multi_channel_dev0.log" 2>/dev/null || true
cat /root/ascend/log/debug/device-2/* > "${LOG_DIR}/multi_channel_dev2.log" 2>/dev/null || true
