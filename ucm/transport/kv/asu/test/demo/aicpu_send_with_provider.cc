 ...
 *
 * This demo shows how to use AICPUTransProvider to manage HCOMM resources
 * and perform data transfer between two processes.
 *
 * Run two processes:
 *   Rank 0: ./aicpu_send_with_provider --rank=0 --logic-dev=1 --phy-dev=5 --ip=193.222.2.6 --bytes=4096 \
 *             --local-file=/tmp/r0.bin --peer-file=/tmp/r1.bin --done-file=/tmp/hixl.done \
 *             --kernel-json=./build/device_build/src/ops/hixl_kernel/libcann_hixl_kernel_custom.json
 *
 *   Rank 1: ./aicpu_send_with_provider --rank=1 --logic-dev=2 --phy-dev=7 --ip=193.222.2.8 --bytes=4096 \
...
constexpr uint32_t kMagic = 0x48585344; // HXSD
constexpr uint32_t kVersion = 1;
constexpr uint16_t kDefaultPort = 16666;
constexpr uint32_t kMailboxMagic = 0x4853524d; // HSRM
...
int WritePeerFile(const std::string &path, const std::string &ip, uint16_t port,
                  const void *desc, uint32_t descLen)
{
    ...
    hdr.descLen = descLen;
    strncpy(hdr.endpointIp, ip.c_str(), sizeof(hdr.endpointIp) - 1);
    hdr.endpointPort = port;
...
int ReadPeerFile(const std::string &path, std::string *ip, uint16_t *port, std::vector<char> *desc)
{
    ...
    *ip = hdr.endpointIp;
    *port = hdr.endpointPort;
    return 0;
}

int WaitForPeerFile(const Options &opt, const std::string &path, 
                    std::string *ip, uint16_t *port, std::vector<char> *desc)
{
    uint32_t attempts = 0;
    for (;;) {
        if (ReadPeerFile(path, ip, port, desc) == 0) {
            return 0;
        }
        ++attempts;
        if (attempts % 50 == 0) {
...
int main(int argc, char **argv)
{
    ...
    Log(opt, "files: local=%s peer=%s done=%s kernelJson=%s",
        opt.localFile.c_str(), opt.peerFile.c_str(), opt.doneFile.c_str(), opt.kernelJson.c_str());

    // Initialize ACL
    Log(opt, "initializing ACL and selecting device");
    CHECK_ACL(aclInit(nullptr));
    CHECK_ACL(aclrtSetDevice(opt.logicDev));

    // Create AICPUTransProvider
    Log(opt, "creating AICPUTransProvider");
    UC::ASU::AICPUTransProvider provider;

    // Allocate device mailbox buffer
    ...
    // Copy message to mailbox (rank 0 only)
    ...
    // Create local endpoint and register memory (before knowing peer)
    Log(opt, "creating local endpoint with phyDev=%u", opt.phyDev);
    std::vector<UC::ASU::TransProvider::ConnectionHandle> connections;
    // Create a temporary connection just for memory registration
    // We'll create the real connection after reading peer file
    CHECK_STATUS(provider.CreateConnection(opt.ip, "0.0.0.0", kDefaultPort, 1, 5000, connections));
    if (connections.empty()) {
        fprintf(stderr, "Failed to create initial connection\n");
        return 1;
    }
    auto connection = connections[0];
    Log(opt, "initial connection created: handle=%p", connection);

    // Register mailbox memory
    Log(opt, "registering mailbox memory with provider");
    ...
    CHECK_STATUS(provider.RegisterMemory(connection, memDescs, memHandles));
    if (memHandles.empty()) {
    ...
    // Get memory token ID
    uint32_t tokenId = 0;
    CHECK_STATUS(provider.GetMemTokenId(memHandle, tokenId));
    Log(opt, "memory token ID: %u", tokenId);

    if (opt.rank == 0) {
        Log(opt, "send mailbox header reserved for AICPU fill: headerAddr=%p expectedMagic=0x%x "
            "expectedState=%u expectedPayloadLen=%llu",
            mailboxBuf, kMailboxMagic, HCOMM_SEND_RECV_MAILBOX_READY,
            static_cast<unsigned long long>(payloadBytes));
    }

    // Export memory descriptor and write to file
    Log(opt, "exporting mailbox descriptor");
    void *exportDesc = nullptr;
    uint32_t exportLen = 0;
    CHECK_STATUS(provider.ExportMemory(connection, memHandle, &exportDesc, &exportLen));
    CHECK_RET(WritePeerFile(opt.localFile, opt.ip, kDefaultPort, exportDesc, exportLen));
    Log(opt, "wrote local endpoint/memory descriptor: path=%s descLen=%u", 
        opt.localFile.c_str(), exportLen);

    // Wait for peer's descriptor (this gives us peer IP and port)
    std::string peerIp;
    uint16_t peerPort;
    std::vector<char> peerDesc;
    Log(opt, "waiting for peer descriptor: path=%s", opt.peerFile.c_str());
    CHECK_RET(WaitForPeerFile(opt, opt.peerFile, &peerIp, &peerPort, &peerDesc));
    Log(opt, "loaded peer descriptor: ip=%s port=%u descLen=%zu", 
        peerIp.c_str(), peerPort, peerDesc.size());

    // Now create the real connection with peer IP
    Log(opt, "creating connection to peer %s:%u", peerIp.c_str(), peerPort);
    std::vector<UC::ASU::TransProvider::ConnectionHandle> realConnections;
    CHECK_STATUS(provider.CreateConnection(opt.ip, peerIp, peerPort, 1, 5000, realConnections));
    if (realConnections.empty()) {
        fprintf(stderr, "Failed to create connection to peer\n");
        return 1;
    }
    auto realConnection = realConnections[0];
    Log(opt, "real connection created: handle=%p", realConnection);

    // Import peer's memory
    Log(opt, "importing peer mailbox memory");
    UC::ASU::TransProvider::MemHandle peerMemHandle = nullptr;
    CHECK_STATUS(provider.ImportMemory(realConnection, peerDesc.data(), 
                                       static_cast<uint32_t>(peerDesc.size()), &peerMemHandle));
    Log(opt, "peer memory imported: handle=%p", peerMemHandle);

    // Get peer memory info
    uint64_t peerMemAddr = 0;
    uint64_t peerMemSize = 0;
    CHECK_STATUS(provider.GetImportedMemoryInfo(realConnection, peerMemHandle, &peerMemAddr, &peerMemSize));
    ...
    sendRecvContext.transportChannel = reinterpret_cast<uint64_t>(realConnection);
    ...
    Log(opt, "creating AICPU send/recv context: transportChannel=%p localMailbox=%p remoteMailbox=0x%llx",
        realConnection, mailboxBuf, static_cast<unsigned long long>(peerMemAddr));
    ...
    Log(opt, "AICPU send/recv context ready: addr=%p size=%zu", 
        sendRecvContextBuf, sizeof(sendRecvContext));

    // Launch kernel
    if (opt.rank == 0) {
        aclrtStream stream = nullptr;
        Log(opt, "creating stream and loading HixlSend kernel");
        CHECK_ACL(aclrtCreateStream(&stream));
        aclrtBinHandle bin = nullptr;
        aclrtFuncHandle func = nullptr;
        CHECK_RET(LoadKernel(opt.kernelJson, "HixlSend", &bin, &func));
        Log(opt, "kernel loaded: bin=%p func=%p", bin, func);
        ...
        Log(opt, "launching HixlSend: sendRecvContext=%p src=%p len=%llu",
            sendRecvContextBuf, args.local_src, static_cast<unsigned long long>(args.len));
        ...
        Log(opt, "sender stream destroyed");
    } else {
        aclrtStream stream = nullptr;
        Log(opt, "creating stream and loading HixlRecv kernel");
        CHECK_ACL(aclrtCreateStream(&stream));
        aclrtBinHandle bin = nullptr;
        aclrtFuncHandle func = nullptr;
        ...
        Log(opt, "launching HixlRecv: sendRecvContext=%p dst=%p capacity=%llu timeoutMs=%u",
            sendRecvContextBuf, args.local_dst,
