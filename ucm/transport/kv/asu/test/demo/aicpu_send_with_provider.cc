...
int main(int argc, char **argv)
{
    ...
    if (opt.rank == 0) {
        ...
        batch.connectionHandle = connection;
        batch.sendBuffer = mailboxBuf;
        batch.len = payloadBytes;
        batch.remoteMailboxAddr = peerMemAddr;
        batch.remoteMailboxSize = peerMemSize;

        Log(opt, "calling provider.Send: mailbox=%p payloadBytes=%llu remoteMailbox=0x%llx remoteSize=%llu",
            mailboxBuf, static_cast<unsigned long long>(payloadBytes),
            static_cast<unsigned long long>(peerMemAddr),
            static_cast<unsigned long long>(peerMemSize));

        auto sendResults = provider.Send({batch}, 0, 0);
        if (sendResults.empty() || !sendResults[0].ok()) {
            fprintf(stderr, "provider.Send failed: %s\n",
